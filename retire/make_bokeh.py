...
for i in range(1, N_years):
    ...
    y_savings.append(
        (y_savings[-1] * (1 + annual_ROI) + annual_savings * y_todays_dollar[i]))
...
plot = figure(plot_width=400, plot_height=400,
              ...
              y_axis_label='Annual net income on interest ($)',
              x_range=(0, N_years))
r1 = plot.line('x', 'y', source=source, line_width=3,
               line_alpha=0.6, line_color='black', legend='annual income on interest')
r2 = plot.line('x', 'y_ri', source=source, line_width=3,
               line_alpha=0.6, line_color='red', legend='retirement income threshold')
r3 = plot.line('x_rt', 'y_rt', source=source, line_width=3,
               line_alpha=0.6, line_color='green', legend='year of retirement')
...
layout = row(
    plot,
    widgetbox(savings_start_slider, annual_savings_slider, retired_income_slider,
              annual_ROI_slider, annual_inflation_slider, retired_tax_rate_slider),
)

# Output and show
output_file("/gh/srcole.github.io/assets/misc/retirement_bokeh.html",
